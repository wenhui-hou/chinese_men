from creat_lattices.trie import Trie
...
class Gazetteer:
    ...
    def insert(self, word_list, source):
        ...
        # print(self.ent2type.items()) 
        #dict_items([('</s>', 'one_source'), (',', 'one_source'), ('的', 'one_source'),...])
        if string not in self.ent2id:
            self.ent2id[string] = len(self.ent2id)
        # print(self.ent2id.items())
        #dict_items([('<UNK>', 0), ('</s>', 1), (',', 2), ('的', 3), ('。', 4),...])

    ...
    for mention, gold in mention_gold.items():
        ...
        for ca in candidate[:args.top_k]:
            if ca not in gold:
                ...
                tokens = mention_tokens + candidate_tokens
                ...
                padding_tokens = ['NONE'] * (args.mentions_max_length + args.candidates_max_length - len(tokens_id))
...
def prepare_instance_bert_test(filename, candidatename,generate_scoresname, args):
    instances = []
    if args.bert_dir=="JianglabSSUMI/TeaBERT":
        wp_tokenizer = XLMRobertaTokenizer.from_pretrained(args.bert_dir)   
    else:
    ...
    mention_gold = pickle.load(open(filename, 'rb'))
    candidates = pickle.load(open(candidatename, 'rb'))
    generate_scores = pickle.load(open(generate_scoresname, 'rb'))
    ...
    for mention, gold in mention_gold.items():
        ...
        all_tokens,all_tokens_id, all_masks, all_segment_ids =[], [], [], []
        for ca in candidate[:args.test_top_k]:
            ...
            all_tokens_id.append(tokens_id)
...
#         g_score = generate_scores[mention]
        g_score = generate_scores[mention][:args.test_top_k]
        
        dict_instance = {'label1': label1,
                         'label2': label2,
                         'tokens': all_tokens,
                         'tokens_id': all_tokens_id,
                         'masks': all_masks,
...
def my_collate_bert(x):
    ...
    return inputs,inputs_id, segment_ids, masks, labels1, labels2
...
def my_collate_bert_test(x):
    ...
    return inputs, inputs_id, segment_ids, masks, labels1, labels2, generate_score
